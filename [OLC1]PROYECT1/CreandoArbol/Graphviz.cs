    ...
    class Graphviz
    {
        public static class Grafo
        {
            ...
            public static Image RenderImage(string source, string format)
            {
                ...
                {
                    //Console.WriteLine(Image.FromStream(stream));
        ...
        public static class FileDotEngine
        {
            ...
            public static Bitmap Run1(String imagen, String dot)
            {
                string executable = @".\external\dot.exe";
                File.WriteAllText(imagen, dot);
                System.Diagnostics.Process process = new System.Diagnostics.Process();
                ...
                // Setup executable and parameters
                process.StartInfo.FileName = executable;
                process.StartInfo.Arguments = string.Format(@"{0} -Tjpg -O", imagen);
                ...
                using (Stream bmpStream = System.IO.File.Open(imagen.Trim() + ".jpg", System.IO.FileMode.Open))
                {
                    Image image = Image.FromStream(bmpStream);
                    bitmap = new Bitmap(image);
                }
